; Generated from m6502.asm by tools/m10toca65.py -- do not edit.
; Build: ca65 -D REALIO=1 (KIM) etc.; see PORTING.md.
.feature ubiquitous_idents ; RORSW=0 defines a macro named ROR
.feature org_per_seg ; segments are absolute (see ORG handling)

; TITLE BASIC M6502 8K VER 1.1 BY MICRO-SOFT
	.include "macro10.inc" ;[was: SEARCH M6502]
; SALL 
	; RADIX 10 [literals converted by m10toca65]	;THROUGHOUT ALL BUT MATH-PAK.

;[removed] $Z:: (PDP-10 simulator start label)
	.segment "ZP"	;START OFF AT LOCATION ZERO.
	.org	0
;;; ----- SWITCHES,MACROS. -----

.ifndef REALIO ;[REALIO selectable via ca65 -D REALIO=n]
REALIO = 4
.endif
;4=APPLE.
;3=COMMODORE.
...
;0=PDP-10 SIMULATING 6502
	.ifndef INTPRC	;INTEGER ARRAYS.
INTPRC = 1
.endif
	.ifndef ADDPRC	;FOR ADDITIONAL PRECISION.
ADDPRC = 1
.endif
	.ifndef LNGERR	;LONG ERROR MESSAGES.
LNGERR = 0
.endif
	.ifdef TIME_OV	;CAPABILITY TO SET AND READ A CLK.
TIME .set TIME_OV
.else
TIME .set 0
.endif
	.ifdef EXTIO_OV	;EXTERNAL I/O.
EXTIO .set EXTIO_OV
.else
EXTIO .set 0
.endif
	.ifdef DISKO_OV	;SAVE AND LOAD COMMANDS
DISKO .set DISKO_OV
.else
DISKO .set 0
.endif
	.ifdef NULCMD_OV	;FOR THE "NULL" COMMAND
NULCMD .set NULCMD_OV
.else
NULCMD .set 1
.endif
	.ifdef GETCMD_OV
GETCMD .set GETCMD_OV
.else
GETCMD .set 1
.endif
	.ifdef RORSW_OV
...
RORSW .set 1
.endif
	.ifdef ROMSW_OV	;TELLS IF THIS IS ON ROM.
ROMSW .set ROMSW_OV
.else
ROMSW .set 1
.endif
	.ifdef CLMWID_OV
CLMWID .set CLMWID_OV
.else
CLMWID .set 14
.endif
	.ifndef LONGI	;LONG INITIALIZATION SWITCH.
LONGI = 1
.endif
	.ifdef STKEND_OV
STKEND .set STKEND_OV
.else
STKEND .set 511
.endif
	.ifdef BUFPAG_OV
BUFPAG .set BUFPAG_OV
.else
BUFPAG .set 0
.endif
	.ifdef LINLEN_OV	;TERMINAL LINE LENGTH.
LINLEN .set LINLEN_OV
.else
LINLEN .set 72
.endif
	.ifdef BUFLEN_OV	;INPUT BUFFER SIZE.
BUFLEN .set BUFLEN_OV
.else
BUFLEN .set 72
.endif
	.ifdef ROMLOC_OV	;ADDRESS OF START OF PURE SEGMENT.
ROMLOC .set ROMLOC_OV
.else
ROMLOC .set $2000
.endif
	.ifdef KIMROM_OV
KIMROM .set KIMROM_OV
.else
KIMROM .set 1
.endif
	.if (ROMSW) = 0
...
;X [REALIO=0 (PDP-10 simulator) block removed]	;START AT 8K.
;X ROMLOC= ^O20000
;X RAMLOC=^O1400
;X [end of removed block]
.endif
	.if (REALIO-3) = 0
	DISKO .set 1
	RAMLOC .set $400
...
.endmacro
	...
	JSR	SYNCHR
.endmacro
.macro	DT STR ;[was: IRPC quote-stripping loop]
	.byte	STR
.endmacro
	.macro	LDWD WD
	LDA	WD
...
.endmacro
	...
	BCC	Q
.endmacro
	.macro	BCSA Q	;THESE ARE USED ON THE 6502 BECAUSE
	BCS	Q
.endmacro
	.macro	BEQA Q	;THERE IS NO UNCONDITIONAL BRANCH
	BEQ	Q
.endmacro
	.macro	BNEA Q
	BNE	Q
...
.endmacro
	...
	BVS	Q
.endmacro
.macro	INCW R ;[%Q generated symbol -> .local]
	.local	INCWSKP
	INC	R
	BNE	INCWSKP
	INC	R+1
INCWSKP:
.endmacro
	.macro	SKIP1	;BIT ZERO PAGE TRICK.
	.byte	$24 ;[was: XWD $200,$24 - low byte only]
.endmacro
	.macro	SKIP2	;BIT ABS TRICK.
	.byte	$2C ;[was: XWD $200,$2C - low byte only]
.endmacro
;X [REALIO=0 (PDP-10 simulator) block removed]
;X PRINTX/SIMULATE/
;X [end of removed block]
	.if (REALIO-1) = 0
	.out "KIM"
...
.endif
; PAGE 
;;; ----- INTRODUCTION AND COMPILATION PARAMETERS. -----
;
;
;--------- ---- -- ---------
...
;COMPLEMENTED BY INPUT OF ^O.
	...
	.if (REALIO-4) = 0	;ROOM FOR APPLE PAGE 0 STUFF.
	.org	80
.endif
...
RNDX:	.byte	128	;LOADED OR FROM ROM.
	...
	.byte	199
	.byte	82
	.if (ADDPRC) <> 0	;ONE MORE BYTE.
	.byte	89
.endif

	.segment "ST01"	;PAGE 1 STUFF COMING UP.
	.org	255
...
FNDFOR:	TSX	;LOAD XREG WITH STK PNTR.
	.repeat 4	;IGNORE ADR(NEWSTT) AND RTS ADDR.
	INX
.endrepeat
...
;X [REALIO=0 (PDP-10 simulator) block removed]
;X DDT:	PLA	;GET RID OF NEWSTT RETURN.
;X PLA
;X HRRZ	14,.JBDDT##
;X JRST	0(14)
;X [end of removed block]
...
COPSTR:
	.if (INTPRC) <> 0	;IF STRING, NO INTFLG.
	PLA
.endif
INPCOM:
	.if (TIME) <> 0
...
.endif
	...
	STY	KIMY	;PRESERVE Y.
.endif
	.if (REALIO-4) = 0	;TURN ON B7 FOR APPLE.
	ORA	#<($80)
.endif
	.if (REALIO) <> 0
...
.endrepeat
.endif
	.if (REALIO-4) = 0	;GET [A] BACK FROM APPLE.
	AND	#<($7F)
.endif
...
READ:	LDXY	DATPTR	;GET LAST DATA LOCATION.
	.byte	$A9 ;[was: XWD $200,$A9 - low byte only]	;LDAI TYA TO MAKE IT NONZERO.
	.if (BUFPAG) = 0
INPCON:
.endif
	TYA
...
FNDOER:	JSR	GETFNM	;GET THE FUNCTION'S NAME.
	...
	BEQ	ERRGUF
	STA	VARPNT+1
	.if (ADDPRC) <> 0	;SINCE DEF USES ONLY 4.
	INY
.endif
DEFSTF:	LDA	(VARPNT),Y
...
;;; ----- FLOATING POINT MATH PACKAGE CONFIGURATION. -----
	...
	; RADIX 8 [literals converted by m10toca65]	;!!!! ALERT !!!!
...
MRCHKR:	LDX	#<($A)
MRCHR:	LDA	$6000,X
;X [IF2 (pass-2 only) block removed]
;X MRCHR:	LDA	SINCON+36,X,
;X [end of removed block]
	AND	#<($3F)
	STA	(POKER),Y
	INY
...
.endif
.endif
	.if (RORSW) <> 0	;ROTATE ARGUMENT 1 BIT RIGHT.
	ROR	A
.endif
	.if (RORSW) = 0
	PLA
...
DIVNRM:	.repeat 6	;GET LAST TWO BITS INTO MSB AND B6.
	ASL	A
.endrepeat
	STA	FACOV
...
FOUTYP:	ADC	#<('0'-1)	;GET A CHARACTER TO PRINT.
	.repeat 3+ADDPRC	;BUMP POINTER UP.
	INY
.endrepeat
	STY	FDECPT
	LDY	FBUFPT
...
;;; ----- SYSTEM INITIALIZATION CODE. -----
	; RADIX 10 [literals converted by m10toca65]	;IN ALL NON-MATH-PACKAGE CODE.
...
;X [REALIO=0 (PDP-10 simulator) block removed]	;MAKE AN INST.
;X HRLI 1,^O1000
;X [end of removed block]
	.if (REALIO-3) <> 0
	STA	START
...
.endif
.endif
.if (ROMSW) <> 0 ;[PATCH: STXY TXTTAB made unconditional, see PORTING.md]
	LDXYI	RAMLOC
.endif
	STXY	TXTTAB
	LDY	#<(0)
...
.endif
;X [REALIO=0 (PDP-10 simulator) block removed]
;X DT"SIMULATED BASIC FOR THE 6502 V1.1"
;X [end of removed block]
	.if (REALIO-1) = 0
	DT	"KIM BASIC V1.1"
.endif
...
;X [end of removed block]
	...
	START = 0
.endif
;[removed] END $Z+START (entry point is the load address)
